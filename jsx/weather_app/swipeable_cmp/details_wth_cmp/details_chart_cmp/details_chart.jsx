...
class DetailsChart extends React.Component{
  ...
  scaleTemps(){
    let temps = this.extractTemps();
    let max = Math.max(...temps);
    let min = Math.min(...temps);
    function normalize(value){
      return Math.trunc(500-(499*((value-min)/(max-min))+1));

    }
    return temps.map(normalize);
  }
  ...
  valuesToSVGPoints(values){
    function reduction(acc, elem, index, full){
      if(index==1){
        return "75 " + (acc+50) + ", 225 " + (elem+50)
      }else{
        return acc + ", " + (150*(index)+75) + " " + (elem+50)
      }
    }
    return values.reduce(reduction);
  }
